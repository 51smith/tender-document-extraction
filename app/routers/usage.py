...
async def get_cost_analysis(
    ...
    try:
        ...
        avg_cost_per_1k_tokens = (total_cost * 1000) / sum(daily_tokens) if sum(daily_tokens) > 0 else 0
...
def _generate_cost_recommendations(avg_daily_cost: float, avg_cost_per_request: float) -> List[str]:
    ...
    if avg_cost_per_request > 0.50:
        recommendations.append("High per-request cost detected - consider optimizing prompt templates")
