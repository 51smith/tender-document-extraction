...
class PromptBuilder:
    ...
    def __init__(
        ...
        configs_dir: Union[str, Path] = None
    ):
    ...
    def build_prompt(
        ...
        if 'system_prompt' in template.content:
            prompt_parts.append(self._render_template_section(
                template.content['system_prompt'], template_vars
            ))
        ...
        if 'task_description' in template.content:
            prompt_parts.append(self._render_template_section(
                template.content['task_description'], template_vars
            ))
        ...
        if 'schema_definition' in template.content:
            ...
            prompt_parts.append(self._render_template_section(
                template.content['schema_definition'], template_vars
            ))
        ...
        if 'quality_checks' in template.content:
            prompt_parts.append(self._render_template_section(
                template.content['quality_checks'], template_vars
            ))
        
        # Output format
        if 'output_format' in template.content:
            prompt_parts.append(self._render_template_section(
                template.content['output_format'], template_vars
            ))
    ...
    def validate_prompt_build(self, prompt_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if prompt_length == 0:
            ...
            validation_results["warnings"].append(f"Prompt is very long: {prompt_length} characters")
