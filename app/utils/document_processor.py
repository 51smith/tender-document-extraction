...
class DocumentProcessor:
    ...
    def process_document(
        ...
        try:
            if file_type == "application/pdf":
                ...
                raise UnsupportedDocumentFormatError(file_type, list(self.SUPPORTED_FORMATS.keys()))
    ...
    def _detect_file_type(
        ...
        raise UnsupportedDocumentFormatError(
            f"unknown (detected from {filename})", list(self.SUPPORTED_FORMATS.keys())
        )

    def _process_pdf(
        self, content: bytes, filename: str, content_hash: str, file_type: str
    ) -> DocumentContent:
        ...
        try:
            ...
            if len(full_text) > self.max_text_length:
                logger.warning(
                    f"Document text truncated from {len(full_text)} to {self.max_text_length} characters"
                )
            ...
            metadata = {
                ...
                "modification_date": getattr(pdf_reader.metadata, "modification_date", None)
                if pdf_reader.metadata
                else None,
            }
    ...
    def _process_docx(
        ...
        try:
            ...
            for table_num, table in enumerate(doc.tables):
                table_data = {"table_number": table_num + 1, "rows": [], "content": ""}
    ...
    def _process_image(
        ...
        try:
            ...
            text = (
                f"[IMAGE: {filename} - {image.width}x{image.height} pixels, {image.format} format]"
            )
    ...
    def _process_text(
        ...
        try:
            # Try different encodings with better heuristics
            text = None

            # First try UTF-8 with strict validation
            try:
                decoded_text = content.decode("utf-8")
                # Check if decoded text contains mostly printable characters
                # to avoid accepting Latin1 content decoded as UTF-8
                printable_ratio = sum(
                    1 for c in decoded_text if c.isprintable() or c.isspace()
                ) / max(len(decoded_text), 1)
                if printable_ratio > 0.95:  # Most characters should be printable
                    text = decoded_text
            except UnicodeDecodeError:
                pass

            # If UTF-8 failed or produced low-quality text, try other encodings
            if text is None:
            ...
            if len(text) > self.max_text_length:
                logger.warning(
                    f"Text document truncated from {len(text)} to {self.max_text_length} characters"
                )
    ...
    def get_multimodal_content(self, document: DocumentContent) -> List[Union[str, Dict[str, Any]]]:
        ...
        for image in document.images:
            content_parts.append({"mime_type": image["content_type"], "data": image["data"]})

        return content_parts

    def validate_document(self, document: DocumentContent) -> Dict[str, Any]:
        """Validate processed document content."""
        validation = {"is_valid": True, "errors": [], "warnings": [], "metrics": {}}
