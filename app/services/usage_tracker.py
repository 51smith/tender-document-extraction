...
class UsageTracker:
    ...
    def __init__(self):
        ...
        self.enabled = settings.monitoring.enable_usage_tracking
    ...
    async def record_usage(
        ...
        logger.info(f"Recorded usage: {total_tokens} tokens, ${cost.total_cost:.4f} for job {job_id}")
    ...
    async def _check_cost_alerts(self):
        ...
        try:
            ...
            if daily_cost:
                ...
                threshold = float(settings.monitoring.cost_alert_threshold)
                
                if daily_cost > threshold:
                    logger.warning(f"Daily cost threshold exceeded: ${daily_cost:.2f} > ${threshold:.2f}")
