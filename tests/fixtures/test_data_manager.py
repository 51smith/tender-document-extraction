...
class TenderDocument:
    ...
    contracting_authority: Dict[str, str]
...
class TestDataManager:
    ...
    def generate_tender_document(
        ...
        domains = {
            "construction": {
                "titles": [
                    "Highway Construction Project", "Bridge Renovation", "Building Construction",
                    "Infrastructure Upgrade", "Road Maintenance", "Tunnel Construction"
                ],
                "authorities": [
                    {"name": "Department of Transportation", "contact": "procurement@dot.gov"},
                    {"name": "Municipal Infrastructure Department", "contact": "tenders@municipality.org"},
            ...
            "technology": {
                "titles": [
                    "IT System Modernization", "Software Development", "Network Upgrade",
                    "Cybersecurity Implementation", "Database Migration", "Cloud Infrastructure"
                ],
                "authorities": [
            ...
            "healthcare": {
                "titles": [
                    "Medical Equipment Procurement", "Healthcare System Implementation",
                    "Hospital Renovation", "Medical Supplies Contract", "Diagnostic Equipment"
                ],
                "authorities": [
        ...
        total_weight = sum(random.uniform(*criteria["weight_range"]) for criteria in selected_criteria)
        ...
        deadline = base_date.strftime("%Y-%m-%dT%H:%M:%SZ")
        
        return TenderDocument(
            project_title=f"{random.choice(domain_data['titles'])} {random.choice(['A', 'B', 'C'])}{random.randint(1, 99)}",
            ...
            description=f"This is a {complexity} {domain} project requiring professional expertise and proven experience.",
            additional_requirements=random.sample(domain_data["requirements"], min(len(domain_data["requirements"]), 3))
        )
...
The contracting authority reserves the right to cancel or modify this tender at any time.
    ...
    def save_test_document(self, document: TenderDocument, filename: str = None) -> Path:
    ...
    def save_expected_extraction(self, expected: ExpectedExtraction, filename: str = None) -> Path:
    ...
    def load_expected_extraction(self, document_hash: str) -> Optional[ExpectedExtraction]:
    ...
    def create_batch_test_files(self, batch_size: int = 5) -> List[Path]:
...
def get_test_dataset(count: int = 5) -> List[Dict[str, Any]]:
    """Get a test dataset."""
