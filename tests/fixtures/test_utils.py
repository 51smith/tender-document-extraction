...
class TestAPIClient:
    ...
    async def upload_file(
        ...
        data = {
            ...
            "enable_multimodal": str(enable_multimodal).lower()
        }
...
def assert_extraction_result_valid(result: Dict[str, Any], min_confidence: float = 0.5):
...
def create_batch_test_files(count: int = 3) -> List[tuple]:
...
def setup_test_environment():
    ...
    with patch('app.services.gemini_service.genai') as mock_genai, \
         patch('app.services.job_manager.redis') as mock_redis, \
         patch('app.utils.document_processor.PyPDF2') as mock_pypdf2, \
         patch('app.utils.document_processor.docx') as mock_docx:
