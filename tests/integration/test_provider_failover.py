...
class TestProviderFailoverScenarios:
    ...
    async def test_cascade_failover_all_providers(
        ...
        await mock_openai_client.post("/test-control/set-failure-mode", json={
            "mode": "quota_exceeded", 
            "probability": 1.0
        })
    ...
    async def test_all_providers_fail_scenario(
        ...
        await mock_ollama_client.post("/test-control/set-failure-mode", json={
    ...
    async def test_batch_processing_with_failover(
        ...
        await mock_gemini_client.post("/test-control/set-failure-mode", json={
            "mode": "rate_limit",
            "probability": 0.6  # 60% chance of failure
        })
...
class TestAdvancedFailoverScenarios:
    ...
    async def test_partial_batch_failover(
        ...
        await mock_gemini_client.post("/test-control/set-failure-mode", json={
            "mode": "safety_filter",
            "probability": 0.4  # 40% chance of safety filter
        })
