...
from tests.fixtures.test_data_manager import test_data_manager, get_sample_document
...
class TestDocumentExtractionWorkflow:
    ...
    async def test_error_handling_workflow(
        ...
        await mock_gemini_client.post("/test-control/set-failure-mode", json={
            "mode": "rate_limit",
            "probability": 1.0
        })
...
class TestJobManagementWorkflow:
    ...
    async def test_job_lifecycle_workflow(
        ...
        assert status_data["status"] == "completed"
        assert "processing" in progress_updates or "running" in progress_updates  # Job went through processing
        
        # Step 5: Download results in different formats
        formats = ["json", "csv", "xlsx"]
        
        for fmt in formats:
            download_response = await test_app_client.get(f"/api/v1/jobs/{job_id}/download?format={fmt}")
            if fmt == "json":
                assert download_response.status_code == 200
...
class TestComplexWorkflowScenarios:
    ...
    async def test_concurrent_requests_workflow(
        ...
        assert successful_responses >= 5, f"Only {successful_responses} out of {len(documents)} requests succeeded"