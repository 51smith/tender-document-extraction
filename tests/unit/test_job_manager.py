...
class TestJobManager:
    ...
    def test_get_sync_connection(self):
        ...
        with patch("rq.Connection") as mock_connection, \
    ...
    @pytest.mark.asyncio
    async def test_create_extraction_job_single_document(self, mock_redis_client, sample_document_request):
    ...
    async def test_store_job(self, mock_redis_client, sample_extraction_job):
        ...
        # Verify expire call
        mock_redis_client.expire.assert_called_with(f"job:{sample_extraction_job.job_id}", 86400 * 7)
    ...
    async def test_enqueue_job_default_queue(self, mock_redis_client, sample_document_request):
        ...
        with patch("rq.Queue") as mock_queue_class, \
             patch.object(job_manager, "_get_sync_connection") as mock_sync_conn, \
    ...
    @pytest.mark.asyncio
    async def test_enqueue_job_high_priority_queue(self, mock_redis_client, sample_document_request):
    ...
    async def test_update_job_status_basic(self, mock_redis_client, sample_extraction_job):
        ...
        with patch.object(job_manager, "get_job") as mock_get_job, \
             patch.object(job_manager, "_store_job") as mock_store_job:
    ...
    async def test_cancel_job_success(self, mock_redis_client, sample_extraction_job):
        ...
        with patch.object(job_manager, "get_job") as mock_get_job, \
             patch.object(job_manager, "update_job_status") as mock_update_status, \
             patch.object(job_manager, "_cleanup_job_data") as mock_cleanup, \
             patch.object(job_manager, "_get_sync_connection") as mock_sync_conn, \
             patch("rq.job.Job.fetch") as mock_job_fetch:
    ...
    async def test_cleanup_job_data(self, mock_redis_client):
        ...
        expected_calls = [
            (("job:test-job-123:files",), {}),
            (("job:test-job-123:rq_id",), {})
        ]
        mock_redis_client.delete.assert_has_calls([
    ...
    async def test_delete_job_success(self, mock_redis_client, sample_extraction_job):
        ...
        with patch.object(job_manager, "get_job") as mock_get_job, \
             patch.object(job_manager, "cancel_job") as mock_cancel:
    ...
    async def test_get_job_statistics(self, mock_redis_client):
        ...
        with patch("rq.Queue") as mock_queue_class, \
            ...
            mock_redis_client.zcount.side_effect = [3, 2, 4, 1]  # pending, processing, completed, failed
