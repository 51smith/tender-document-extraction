...
class TestExtractionService:
    ...
    def mock_dependencies(self):
        ...
        with patch("app.services.extraction_worker.get_llm_service") as mock_llm, \
             patch("app.services.extraction_worker.get_document_processor") as mock_doc_proc, \
             patch("app.services.extraction_worker.get_prompt_builder") as mock_prompt, \
             patch("app.services.extraction_worker.get_job_manager") as mock_job_mgr, \
             patch("app.services.extraction_worker.get_gemini_client") as mock_gemini:
            
            # Setup LLM service mock
            mock_llm_instance = MagicMock()
            mock_llm_instance.get_provider_name.return_value = "openai"
            mock_llm_instance.model = "gpt-4"
            mock_llm_instance.get_provider_info.return_value = {"provider": "openai", "model": "gpt-4"}
    ...
    @pytest.mark.asyncio
    async def test_process_document_content_success(self, mock_dependencies, mock_document_content, mock_extraction_request):
        """Test successful document content processing."""
        # Setup mocks
        mock_dependencies["document_processor"].process_document.return_value = mock_document_content
    ...
    @pytest.mark.asyncio
    async def test_process_document_content_with_validation_error(self, mock_dependencies, mock_document_content, mock_extraction_request):
        ...
        result = await service._process_document_content(b"invalid content", mock_extraction_request)
    ...
    @pytest.mark.asyncio
    async def test_process_document_content_with_multimodal_gemini(self, mock_dependencies, mock_extraction_request):
        ...
        mock_gemini_response = {
            "extracted_data": {
                "project_title": "Multimodal Project",
                "contract_type": "works"
            },
            "confidence_scores": {
                "project_title": 0.95,
    ...
    @pytest.mark.asyncio
    async def test_process_document_content_multimodal_fallback(self, mock_dependencies, mock_extraction_request):
        ...
        mock_ai_response = {
            "response": json.dumps({
                "extracted_data": {
                    "project_title": "Text-only Project"
                }
            })
        }
        mock_dependencies["llm_service"].generate_content = AsyncMock(return_value=mock_ai_response)
    ...
    @pytest.mark.asyncio
    async def test_process_document_content_extraction_error(self, mock_dependencies, mock_document_content, mock_extraction_request):
        ...
        mock_dependencies["llm_service"].generate_content = AsyncMock(side_effect=GeminiAPIException("API Error"))
    ...
    async def test_process_multiple_documents_gemini_path(self, mock_dependencies):
        ...
        documents = {
            "doc1.pdf": b"Content 1",
            "doc2.pdf": b"Content 2"
        }
        ...
        mock_gemini_response = {
            "extracted_data": {
                "project_title": "Combined Project"
            },
            "confidence_scores": {
                "project_title": 0.9,
                "overall": 0.85
        ...
        mock_dependencies["gemini_client"].process_multiple_documents = AsyncMock(return_value=mock_gemini_response)
        ...
        mock_doc_content = DocumentContent(
            ...
            content_hash="hash", file_type="application/pdf", file_size=1000
        )
    ...
    async def test_process_multiple_documents_non_gemini_path(self, mock_dependencies):
        ...
        mock_doc_content = DocumentContent(
            text="Processed document text",
            images=[], tables=[], metadata={}, content_hash="hash",
    ...
    def test_parse_ai_response_full_format(self):
        ...
        mock_document = DocumentContent(
            text="Test content", images=[], tables=[], metadata={},
            content_hash="hash", file_type="application/pdf", file_size=1000
        )
    ...
    def test_parse_ai_response_minimal_format(self):
        ...
        mock_document = DocumentContent(
            text="Test content", images=[], tables=[], metadata={},
            content_hash="hash", file_type="text/plain", file_size=500
        )
    ...
    def test_assess_complexity_simple(self):
        ...
        simple_doc = DocumentContent(
            ...
            images=[], tables=[], metadata={"page_count": 1},
            content_hash="hash", file_type="text/plain", file_size=1000
        )
        
        simple_response = {
            "extracted_data": {"project_title": "Simple"}
        }
    ...
    def test_assess_complexity_complex(self):
        ...
        complex_doc = DocumentContent(
            ...
            content_hash="hash", file_type="application/pdf", file_size=100000
        )
        ...
        complex_response = {
            ...
            "_metadata": {
                "processing_time": 70  # >60s = +2 complexity
            }
        }
...
class TestWorkerFunctions:
    ...
    async def test_process_job_async_batch_request(self):
        ...
        with patch("app.services.extraction_worker.ExtractionService") as mock_service_class, \
             patch("app.services.job_manager.JobManager") as mock_job_manager_class, \
             patch("app.services.extraction_worker.BatchExtractionRequest") as mock_batch_req:
    ...
    def test_process_extraction_job_with_error(self):
        ...
        with patch("app.services.extraction_worker._process_job_async") as mock_async_process, \
             patch("app.services.extraction_worker._handle_job_error") as mock_handle_error:
