...
class TestUsageTracker:
    ...
    def mock_redis(self):
        ...
        # Pipeline should return a mock pipeline object, not be an AsyncMock itself
        mock_pipeline = Mock()
        mock_pipeline.hincrby = Mock()
        mock_pipeline.hincrbyfloat = Mock()
        mock_pipeline.expire = Mock()
        mock_pipeline.execute = AsyncMock()  # execute() is async
        # pipeline() is synchronous method that returns the pipeline object
        mock.pipeline = Mock(return_value=mock_pipeline)
        return mock
    ...
    async def test_update_metrics(self, usage_tracker, mock_redis):
        ...
        # Verify pipeline operations (using the fixture's mock_pipeline)
    ...
    @pytest.mark.asyncio()
    async def test_check_cost_alerts_triggered(self, mock_redis):
        ...
        with patch("app.services.usage_tracker.settings") as mock_settings:
            ...
            # Configure mock to return cost above threshold for today's key
            def mock_hget(key, field):
                if "usage:daily:" in key and field == "cost_usd":
                    return "75.00"  # Above threshold of 50
                return None
...
class TestUsageTrackerIntegration:
    ...
    async def test_cost_alert_threshold_monitoring(self, mock_redis):
        ...
        with patch("app.services.usage_tracker.settings") as mock_settings:
            ...
            # Mock high daily cost for specific key lookup
            def mock_hget(key, field):
                if "usage:daily:" in key and field == "cost_usd":
                    return "15.00"  # Above threshold of 10.0
                return None
