...
class TestPromptValidation:
    ...
    def extraction_service(self, mock_gemini_client):
        ...
        with patch('app.services.extraction_worker.get_gemini_client', return_value=mock_gemini_client):
    ...
    async def test_golden_dataset_validation(self, prompt_validation_dataset, extraction_service):
        ...
        for test_case in prompt_validation_dataset:
            ...
            with patch('app.services.extraction_worker.get_document_processor') as mock_processor:
                ...
                mock_doc_processor.get_multimodal_content.return_value = [test_case["document_text"]]
                
                # Mock Gemini response to return expected data
                mock_response = self._create_mock_response_from_expected(test_case["expected_fields"])
                
                with patch.object(extraction_service.gemini_client, 'generate_content', return_value=mock_response):
                    # Extract data
                    result = await extraction_service.extract_from_document(
                        test_case["document_text"].encode(),
                        request
                    )
    ...
    def _create_mock_response_from_expected(self, expected_fields: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_extraction_result(self, result, test_case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field_path, expected_value in expected_fields.items():
            ...
            if actual_value is None:
                ...
                errors.append(f"Field mismatch {field_path}: expected {expected_value}, got {actual_value}")
    ...
    async def test_prompt_performance_metrics(self):
        ...
        assert (complex_validation["metrics"]["prompt_length"] > 
