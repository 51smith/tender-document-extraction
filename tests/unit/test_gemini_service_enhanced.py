...
class TestGeminiClientBasics:
    ...
    def test_get_usage_stats(self, mock_settings, mock_genai, mock_get_llm_service):
...
class TestGeminiProviderRouting:
    ...
    @patch("app.services.gemini_service.get_llm_service")
    @pytest.mark.asyncio()
    async def test_multimodal_processing_non_gemini_provider(self, mock_get_llm_service):
    def test_multimodal_processing_non_gemini_provider(self, mock_get_llm_service):
        ...
        with patch.object(GeminiClient, "_initialize_client"):
            client = GeminiClient()
            ...
            with patch("app.services.gemini_service.settings") as mock_settings:
                ...
                with pytest.raises(
                    ValueError,
                    match="Multi-document processing with File API only available for Gemini provider",
                ):
                    await client.process_multiple_documents("Test prompt", documents)
        client = GeminiClient()
