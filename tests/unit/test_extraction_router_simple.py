...
def completed_extraction_job():
    ...
    # Create proper TenderExtractionResult
    result = TenderExtractionResult(
        ...
        confidence_scores=ConfidenceScores(
            project_title=0.95, estimated_value=0.90, submission_deadline=0.85, overall=0.90
        ),
        extraction_notes=ExtractionNotes(
            ambiguities=[], assumptions=[], missing_information=[], recommendations=[]
        ),
        processing_metadata=ProcessingMetadata(
            processing_time=2.5, extraction_complexity="simple", language="en", total_pages=5
        ),
    )
...
class TestJobListingFunction:
    ...
    async def test_list_jobs_limit_capped(self, mock_get_job_manager, mock_job_manager):
        ...
        # Execute
        await list_jobs(limit=200, job_manager=mock_job_manager)
