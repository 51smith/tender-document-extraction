...
class PerformanceTestRunner:
    ...
    async def _run_single_test(self, config: PerformanceTestConfig) -> Dict:
        ...
        cmd = [
            ...
            "--csv", str(self.results_dir / config.test_name),
            ...
            for line in lines[1:]:  # Skip header
                ...
                if len(parts) >= 10 and parts[0] != "Aggregated":
                    stats[parts[0]] = {
                        ...
                        "avg_response_time": float(parts[7]) if parts[7].replace('.', '').isdigit() else 0,
                        "max_response_time": float(parts[9]) if parts[9].replace('.', '').isdigit() else 0
                    }
    ...
    def _validate_test_results(self, config: PerformanceTestConfig, result: Dict) -> None:
        ...
        if parsed_stats:
            ...
            if total_requests > 0:
                ...
                avg_response_times = [s.get("avg_response_time", 0) for s in parsed_stats.values() if s.get("avg_response_time", 0) > 0]
                avg_response_time = sum(avg_response_times) / len(avg_response_times) if avg_response_times else 0
                
                # Validate metrics
                if failure_rate <= config.expected_failure_rate:
                    print(f"✅ {config.test_name} failure rate: {failure_rate:.2%} (expected ≤ {config.expected_failure_rate:.2%})")
                else:
                    print(f"❌ {config.test_name} failure rate: {failure_rate:.2%} (expected ≤ {config.expected_failure_rate:.2%})")
                
                if avg_response_time <= config.expected_avg_response_time:
                    print(f"✅ {config.test_name} avg response time: {avg_response_time:.0f}ms (expected ≤ {config.expected_avg_response_time:.0f}ms)")
                else:
                    print(f"❌ {config.test_name} avg response time: {avg_response_time:.0f}ms (expected ≤ {config.expected_avg_response_time:.0f}ms)")
